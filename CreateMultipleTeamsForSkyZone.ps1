# This new version addresses the following issues with the original script:

# The file path for the second CSV file was was not working with the name "gs_list" (Changed instead to MemberList).
# The variable names were not very descriptive, so they were renamed to something more meaningful.
# The loop for the second CSV file was inside the loop for the first CSV file, which is not what was intended. The second loop was moved outside the first loop.
# The group ID property for the team was accessed incorrectly ($team.Id instead of $team.GroupId).
# The exception message was not very informative, so it was updated to provide more details about the error.

try {
    # Set CSV file paths. Change these locations accordingly.
    $teamFilePath = "C:\temp\BulkTeams.csv"
    $memberFilePath = "C:\temp\MemberList.csv"
